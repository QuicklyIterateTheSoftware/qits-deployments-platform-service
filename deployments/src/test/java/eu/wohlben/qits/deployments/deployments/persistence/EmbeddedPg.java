 ...
 * with no docker, and the component's store is postgres now — so Testcontainers and Quarkus dev
 * services are both out, and the schema still has to be exercised against the database it actually
 * ships on ({@code unique nulls not distinct} exists on no other). Zonky resolves real postgres
 * binaries as ordinary Maven artifacts and this class spawns them as a child process: a dependency,
 * not a daemon.
 *
 * <p><b>It is copied per module rather than shared</b>, because sharing it would mean a test-jar
 * dependency between two modules that deliberately have none. This copy serves {@code
 * PdSchemaTest}; {@code service} carries the other, with a config source beside it that hands the
 * instance to every {@code @QuarkusTest}. The databases are named per module for the same reason a
 * container would be — so two suites cannot mean the same name.
 *
...
public final class EmbeddedPg {
  ...
  /** The admin url — the {@code postgres} database, which is where a provisioner connects. */
